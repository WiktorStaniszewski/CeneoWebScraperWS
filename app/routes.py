            ...
            if opinions_count:
                ...
                while (url):
                    ...
                    product = {
                        ...
                        "recommendation_distribution" : opinions.recommendation.value_counts(dropna=False).reindex(["Polecam",None,"Nie polecam"]).to_dict()
                    }
                    if not os.path.exists("app/data/products"):
                        os.mkdir('app/data/products')    
                    with open(f"app/data/products/{product_id}.json", 'w', encoding="UTF-8") as jfile:
                        json.dump(product, jfile, indent=3, ensure_ascii=False)
...
def products():
    ...
    for product_id in products_list:
        with open(f"app/data/products/{product_id}.json", 'r', encoding="UTF-8") as jfile:
